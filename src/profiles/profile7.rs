...
impl Profile7 {
    ...
    fn validate_config(config: &Profile7Config) -> E2EResult<()> {
        validation::validate_min_data_length_u32(config.min_data_length, 20 * BITS_PER_BYTE)?;
    ...
    fn write_data_length(&self, data: &mut [u8]) {
        ...
        field_ops::write_be_u32_at(data, offset + 8, data.len() as u32);
    }
    fn write_counter(&self, data: &mut [u8]) {
        let offset = field_ops::calculate_offset_bytes_u32(self.config.offset);
        field_ops::write_be_u32_at(data, offset + 12, self.counter);
    }
    fn write_data_id(&self, data: &mut [u8]) {
        let offset = field_ops::calculate_offset_bytes_u32(self.config.offset);
        field_ops::write_be_u32_at(data, offset + 16, self.config.data_id);
    }
    ...
    fn write_crc(&self, calculated_crc: u64, data: &mut [u8]) {
        let offset = field_ops::calculate_offset_bytes_u32(self.config.offset);
        field_ops::write_be_u64_at(data, offset, calculated_crc);
    }
    ...
    fn read_data_id(&self, data: &[u8]) -> u32 {
        let offset = field_ops::calculate_offset_bytes_u32(self.config.offset);
        field_ops::read_be_u32_at(data, offset + 16)
    }
    fn read_crc(&self, data: &[u8]) -> u64 {
        let offset = field_ops::calculate_offset_bytes_u32(self.config.offset);
        field_ops::read_be_u64_at(data, offset)
    }
