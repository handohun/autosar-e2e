...
impl Profile7 {
    ...
    fn read_data_id(&self, data: &[u8]) -> u32 {
        let offset = (self.config.offset / BITS_PER_BYTE) as usize;
        u32::from_be_bytes([data[offset + 16], data[offset + 17], data[offset + 18], data[offset + 19]])
    }
    fn read_crc(&self, data: &[u8]) -> u64 {
        let offset = (self.config.offset / BITS_PER_BYTE) as usize;
        u64::from_be_bytes([data[offset], data[offset + 1], data[offset + 2], data[offset + 3], data[offset + 4], data[offset + 5], data[offset + 6], data[offset + 7]])
    }
...
impl E2EProfile for Profile7 {
    ...
    fn check(&mut self, data: &[u8]) -> E2EResult<E2EStatus> {
        // Check data length
        self.validate_length(data.len() as u32)?;
        let check_items = Profile7Check{rx_data_length: self.read_data_length(data), 
